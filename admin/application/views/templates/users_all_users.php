	...
	<table id="table-sm" class="table table-bordered table-hover table-sm">
		...
		<tbody>
			...
			<tr id="trrow_<?=$row['admin_id']?>">
				<td><?=$key+1?></td>
				<td><img class="img-responsive img-circle" src="<?=main_url()?>uploads/admins/<?=$row['admin_photo']?>" style="max-width: 30px;"> 
					<?=$row['admin_name']?>
				...
				<td class="">
					<div class="tabledit-toolbar btn-toolbar" style="text-align: left;">
                       <div class="btn-group btn-group-sm" style="float: none;">
                       		...
                       		<a href="<?=base_url()?>users/edit_user/<?=$row['admin_id']?>" class="tabledit-edit-button btn btn-sm btn-default" style="float: none;"><span class="glyphicon glyphicon-pencil"></span></a>
                       		<?php } ?>
                       		<?php if(
		...
		function(isConfirm) {
			if (isConfirm) {	
				...
				$.ajax({
					type: "POST",
					dataType: "json",
					url:'<?=base_url()?>users/api_delete_user',
					data:{id:id_to_del},
