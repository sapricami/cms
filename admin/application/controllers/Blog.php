...
class Blog extends CI_Controller {
	...
	public function __construct()
	{
		...
		if(!isset($this->data['view_permissions']->blog)){show_404();}
	}
	...
	public function index()
	{
		$this->data['page_data']=array(
			...
			'breadcrumb' => array(
				...
				array('link'=>'blog','title'=>'Blog','active'=>true,),
			),
		);

		$sql = $this->db->get_where('cms_modules',array('module_slug'=>'blog'));
		$row = $sql->row();
		$this->data['module_status'] = $row->module_status;

		$this->data['current_slug']='blog';
		$this->common->layout('blog',$this->data);		
	}
	public function api_toggle_module(){
		...
		if(isset($this->data['modify_permissions']->blog)&&($this->data['modify_permissions']->blog)){


			if($this->db->update('cms_modules',array('module_status'=>$change_to),array('module_slug'=>'blog'))){
