...
class Visitors extends CI_Controller {
	...
	public function __construct()
	{
		...
		if(!isset($this->data['view_permissions']->visitors)){show_404();}
	}

	public function index()
	{
		$this->data['page_data']=array(
			'meta_title' => 'Admin/Visitors',
			'meta_description' => '',
			'meta_keywords' => '',
			'page_title' => 'Visitors',
			'breadcrumb' => array(
				array('link'=>'dashboard','title'=>'Dashboard','active'=>false,),
				array('link'=>'visitors','title'=>'Visitors','active'=>true,),
			),
		);

		$this->data['module_status'] = $this->data['modules']['visitors']['module_status'];

		$this->data['current_slug']='visitors';
		$this->common->layout('visitors',$this->data);		
	}
	public function api_toggle_module(){
		header('Content-type: application/json');
		$response = array();

		$status = $this->data['modules']['visitors']['module_status'];
		$change_to = ($status=='1')?'0':'1';

		if(isset($this->data['modify_permissions']->visitors)&&($this->data['modify_permissions']->visitors)){


			if($this->db->update('cms_modules',array('module_status'=>$change_to),array('module_slug'=>'visitors'))){
