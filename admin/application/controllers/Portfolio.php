...
class Portfolio extends CI_Controller {
	...
	public function __construct()
	{
		...
		if(!isset($this->data['view_permissions']->portfolio)){show_404();}
	}

	public function index()
	{
		$this->data['page_data']=array(
			'meta_title' => 'Admin/Portfolio',
			'meta_description' => '',
			'meta_keywords' => '',
			'page_title' => 'Portfolio',
			'breadcrumb' => array(
				array('link'=>'dashboard','title'=>'Dashboard','active'=>false,),
				array('link'=>'portfolio','title'=>'Portfolio','active'=>true,),
			),
		);

		$sql = $this->db->get_where('cms_modules',array('module_slug'=>'portfolio'));
		$row = $sql->row();
		$this->data['module_status'] = $row->module_status;

		$this->data['current_slug']='portfolio';
		$this->common->layout('portfolio',$this->data);		
	}
	public function api_toggle_module(){
		...
		if(isset($this->data['modify_permissions']->portfolio)&&($this->data['modify_permissions']->portfolio)){


			if($this->db->update('cms_modules',array('module_status'=>$change_to),array('module_slug'=>'portfolio'))){
