...
class Pages extends CI_Controller {
	...
	public function index()
	{
		$this->data['page_data']=array(
			...
			'breadcrumb' => array(
				...
				array('link'=>'pages','title'=>'Pages','active'=>true,),
			),
		);
		if($this->input->post()){
			...
			if ($this->form_validation->run() == TRUE) {
				...
				redirect('pages/edit_page/'.$this->db->insert_id(),'refresh');
			}
		}
		...
		$this->common->layout('pages',$this->data);		
	}
	public function edit_page($id=null)
	{
		$this->data['page_data']=array(
			...
			'breadcrumb' => array(
				array('link'=>'dashboard','title'=>'Dashboard','active'=>false,),
				array('link'=>'pages','title'=>'Pages','active'=>false,),
				array('link'=>'pages/all_pages','title'=>'All Pages','active'=>false,), 
				array('link'=>'pages/all_pages','title'=>'Edit Page','active'=>true,), 
			),
		);
		...
		$this->common->layout('pages_editpages',$this->data);		
	}
	public function edit_page_content($id)
	{
		...
		if ($this->input->post()) {
			...
			if (($this->db->update('cms_pages',$data_db,array('page_id'=>$id)))) {
				$this->session->set_flashdata('message',alert('success','Information Updated'));
				redirect('pages/edit_page_content/'.$id,'refresh');
			} else {
				$this->session->set_flashdata('message',alert('danger','Error Updating'));
				redirect('pages/edit_page_content/'.$id,'refresh');
			}
		...
		$this->data['message']=$this->session->flashdata('message');
		$this->load->view('templates/pages_editpagecontent',$this->data);	
	}
